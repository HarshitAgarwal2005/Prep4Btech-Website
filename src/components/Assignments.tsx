...
// //                   <button
// //                     onClick={goBack}
// //                     className="bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 p-2 rounded-lg transition-colors"
// //                   >
...
// //                     className="w-full pl-10 pr-4 py-3 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-slate-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
// //                   />
...
// //           {selectedSubject && (
// //             <div className="bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-xl p-8 border border-slate-200 dark:border-slate-700">
// //               <div className="flex items-center justify-between mb-8">
// //                 <div>
...
// //                         : 'bg-slate-200 text-gray-700 hover:bg-slate-300 dark:bg-slate-700 dark:text-gray-300 dark:hover:bg-slate-600'
// //                     }`}
// //                   >
...
// //                       <div
// //   key={content.id}
// //   className="bg-slate-100 rounded-xl p-6 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-300 border border-slate-200 dark:border-slate-600 dark:hover:border-slate-500"
// // >
...
// import { useSearchParams, useNavigate } from 'react-router-dom';
// import { 
//   FileText, Calendar, Clock, Search, Users, BookOpen, Trophy, 
//   ArrowLeft, ChevronRight, Play, Book, FlaskConical, Video, 
//   ClipboardList, GraduationCap, Eye, Filter 
// } from 'lucide-react';

// // --- IMPORTS ---
// // 1. Keep 'branches' (static UI data). Remove contentSubjects/contentItems.
// import { branches } from '../data/contentData'; 
// // 2. Import your new hooks
// import { useSubjects, useSubjectContent } from '../hooks/useContent';
// // 3. Import Types and Loader
// import { ContentSubject, ContentItem } from '../types';
// import LoadingSkeleton from './ui/LoadingSkeleton'; 
...
//   // 1. Fetch Subjects: Automatically re-fetches when Year, Sem, or Branch changes
//   const { 
...
//   // Handle Search: Filter the *fetched* subjects by the search box text
//   const displaySubjects = fetchedSubjects.filter(subject => 
//     searchQuery 
//       ? subject.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
//         subject.code.toLowerCase().includes(searchQuery.toLowerCase())
//       : true
...
//     if (semester) setSelectedSemester(parseInt(semester));
//   }, [searchParams]);
...
//     if (selectedSemester) items.push(`Sem ${selectedSemester}`);
//     if (selectedBranch) {
//       // Find branch name from the static list
//       const branch = branches.find(b => b.code === selectedBranch);
//       if (branch) items.push(branch.code);
//       else items.push(selectedBranch);
//     }
//     if (selectedSubject) items.push(selectedSubject.name);
//     return items.join(' > ');
//   };
...
//             </div>
//             <p className="text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
//               Access content organized by year, semester, and subject.
//             </p>
//           </div>
...
//               <div className="flex items-center justify-between">
//                 <div className="flex items-center space-x-4">
//                   <button onClick={goBack} className="bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 p-2 rounded-lg">
...
//                   </div>
//                 </div>
//                 <button onClick={resetSelection} className="bg-red-100 text-red-600 py-2 px-4 rounded-lg text-sm">
//                   Reset All
//                 </button>
//               </div>
//             </div>
...
//                   value={searchQuery}
//                   onChange={(e) => setSearchQuery(e.target.value)}
//                   className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-700 border rounded-lg focus:ring-2 focus:ring-blue-500"
//                 />
//                 <Search className="absolute left-3 top-3.5 h-5 w-5 text-gray-400" />
//               </div>
...
//                       key={subject.id}
//                       onClick={() => setSelectedSubject(subject)}
//                       className="bg-slate-100 hover:bg-slate-200 dark:bg-slate-700/50 dark:hover:bg-slate-700 rounded-2xl p-6 cursor-pointer transition-all hover:scale-105 border border-slate-200 dark:border-slate-600 group"
//                     >
//                       <div className="text-center">
//                         <div className="bg-blue-600 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
//                           <span className="text-2xl">{subject.icon || '📚'}</span>
...
//                   <BookOpen className="h-16 w-16 text-gray-600 mx-auto mb-4" />
//                   <h3 className="text-xl font-semibold mb-2">No Subjects Found</h3>
//                   <p className="text-gray-400">Try adjusting the year or branch selection.</p>
//                 </div>
//               )}
//             </div>
//           )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {!selectedYear && (
            <div className="bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-lg p-6 mb-8 border border-slate-200 dark:border-slate-700">
              ...
              {/* Year Cards Grid - USING DESIGN FROM COMMENT SECTION */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 {/* First Year */}
                 <div className="bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl p-6 text-white">
                   <div className="flex items-center justify-between mb-4">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     ...
                     >
                       <div className="text-lg font-semibold">Semester 1</div>
                       <div className="text-sm text-blue-200">View Subjects</div>
                     </button>
                     <button
                       onClick={() => { setSelectedYear(1); setSelectedSemester(2); }}
                       className="bg-white/20 hover:bg-white/40 dark:bg-slate-800/30 dark:hover:bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 transition-all duration-300 hover:scale-105"
                 ...
                 <div className="bg-gradient-to-br from-green-600 to-emerald-600 rounded-2xl p-6 text-white">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     ...
                     >
                       <div className="text-lg font-semibold">Semester 3</div>
                       <div className="text-sm text-green-200">View Subjects</div>
                     </button>
                     <button
                       onClick={() => { setSelectedYear(2); setSelectedSemester(4); }}
                       className="bg-white/20 hover:bg-white/40 dark:bg-slate-800/30 dark:hover:bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 transition-all duration-300 hover:scale-105"
                 ...
                 <div className="bg-gradient-to-br from-purple-600 to-pink-600 rounded-2xl p-6 text-white">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     ...
                     >
                       <div className="text-lg font-semibold">Semester 5</div>
                       <div className="text-sm text-purple-200">View Subjects</div>
                     </button>
                     <button
                       onClick={() => { setSelectedYear(3); setSelectedSemester(6); }}
                       className="bg-white/20 hover:bg-white/40 dark:bg-slate-800/30 dark:hover:bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 transition-all duration-300 hover:scale-105"
                 ...
                 <div className="bg-gradient-to-br from-orange-600 to-red-600 rounded-2xl p-6 text-white">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     ...
                     >
                       <div className="text-lg font-semibold">Semester 7</div>
                       <div className="text-sm text-orange-200">View Subjects</div>
                     </button>
                     <button
                       onClick={() => { setSelectedYear(4); setSelectedSemester(8); }}
                       className="bg-white/20 hover:bg-white/40 dark:bg-slate-800/30 dark:hover:bg-slate-800/50 backdrop-blur-sm rounded-lg p-4 transition-all duration-300 hover:scale-105"
                     >
                       <div className="text-lg font-semibold">Semester 8</div>
                       <div className="text-sm text-orange-200">View Subjects</div>
                     </button>
                   </div>
                 </div>
            ...
            <div className="bg-slate-100/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-xl p-8 mb-12 border border-slate-200 dark:border-slate-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {branches.map((branch) => (
                  ...
                  >
                    <div className="text-center">
                      <div className={`bg-gradient-to-r ${branch.color} w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4`}>
                        <span className="text-2xl">{branch.icon}</span>
                      </div>
                      <h3 className="text-lg font-semibold mb-2">{branch.code}</h3>
                      <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{branch.name}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-500 line-clamp-2">{branch.description}</p>
                    </div>
                  </div>
                ))}
              </div>
